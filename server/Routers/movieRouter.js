...
router.get("/:sortBy/:sortOrder", async (req, res) => {
    try {
        const { sortBy, sortOrder } = req.params
    
        // Sıralama türü kontrolü
        if (sortOrder !== 'asc' && sortOrder !== 'desc') {
            return res.status(400).json({ msg: 'Invalid sortOrder parameter. Use "asc" or "desc".' });
        }
        ...
        try {
            // MongoDB sorgusu: Filmleri sıralayarak getir
            const sortedMovies = await Movie.find().sort(sortQuery);
            res.status(200).json(sortedMovies);
        } catch (error) {
            console.error('Error while fetching sorted movies:', error);
            throw new Error('Error while fetching sorted movies');
        }

    } catch (error) {
        console.error(error);
        res.status(500).json({ msg: 'Error fetching sorted movies' });
    }
});
